 ...
 * <p>Description: Scroll pane that contains a SQL text pane.
 * </p>
...
public class SQLTextArea extends JScrollPane implements DocumentListener {
  ...
  /**
   * @param pos char index currently added/deleted
  ...
  public void changedUpdate(DocumentEvent e) {
   ...
   /**
    * Method called by document listener to update text color, according to SQL syntax.
    */
...
//     setCaretPosition(startpos+text.length());//HAE: Added by myself
     ...
     // search for patterns...
     String pattern = null;
     String[] patterns = new String[] {"SELECT","FROM","WHERE","ORDER BY","GROUP BY","HAVING","IS","NOT","NULL","CREATE", "ALTER", "DROP", "ADD", "TABLE", "INDEX", "FOREIGN","KEY", "REFERENCES", "CONSTRAINT", "PRIMARY","ON","INTO","UNIQUE", "AND", "OR"};
...
//           setCaretPosition(text.length());//HAE: Added by myself
     ...
     // search for types...
     patterns = new String[] {"NUMBER","INTEGER","INT","DOUBLE","DECIMAL","NUMERIC","VARCHAR","VARCHAR2","CHAR","DATE","TIMESTAMP","DATETIME","BOOLEAN","LONG","BLOB","CLOB","LONGRAW","REAL","FLOAT","LONGVARCHAR","SMALLINT","LONGVARBINARY","BIGINT"};
     for(int i=0;i<patterns.length;i++) {
       pos = 0;
       pattern = patterns[i];
       while ((pos = text.indexOf(pattern, pos)) >= 0) {
