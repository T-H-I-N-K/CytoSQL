...
public class SQLTextArea extends JScrollPane implements DocumentListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3239931163292640323L;
	...
	/** char pos to use to start text analysis */
	 ...
	 * @return char pos to use to start analysis
	 */
	private int getStartPos(int pos) {
		try {
			for (int i = pos - 1; i > 0; i--) {
				if (i < 0)
					return 0;
				else if (editor.getText().charAt(i) == ' '
						|| editor.getText().charAt(i) == '\t'
						|| editor.getText().charAt(i) == '\n') {
					return i + 1;
				}
			}
	 ...
	 * syntax.
	 */
	private synchronized void highlight(int argCaretPos) {
		...
		try {
			...
			// search for patterns...
			String pattern = null;
			String[] patterns = new String[] { "SELECT", "FROM", "WHERE",
					"ORDER BY", "GROUP BY", "HAVING", "IS", "NOT", "NULL",
					"CREATE", "ALTER", "DROP", "ADD", "TABLE", "INDEX",
					"FOREIGN", "KEY", "REFERENCES", "CONSTRAINT", "PRIMARY",
					"ON", "INTO", "UNIQUE", "AND", "OR" };
				...
				while ((pos = text.indexOf(pattern, pos)) >= 0) {
					if ((pos == 0 || pos > 0
							&& (text.charAt(pos - 1) == ' '
									...
									|| text.charAt(pos - 1) == '\n' || text
									.charAt(pos - 1) == '('))
							&& (pos + pattern.length() == text.length() || pos
									+ pattern.length() < text.length()
									&& (text.charAt(pos + pattern.length()) == ' '
											|| text.charAt(pos
													+ pattern.length()) == '\t'
											|| text.charAt(pos
													+ pattern.length()) == '\n' || text
											.charAt(pos + pattern.length()) == '('))) {
						editor.getDocument().remove(pos, pattern.length());
						// setCaretPosition(pos);//HAE: Added by myself
						editor.getDocument().insertString(pos, pattern,
								getStyle("reservedWord"));
						// setCaretPosition(text.length());//HAE: Added by
						// myself
			...
			// search for types...
			patterns = new String[] { "NUMBER", "INTEGER", "INT", "DOUBLE",
					"DECIMAL", "NUMERIC", "VARCHAR", "VARCHAR2", "CHAR",
					"DATE", "TIMESTAMP", "DATETIME", "BOOLEAN", "LONG", "BLOB",
					"CLOB", "LONGRAW", "REAL", "FLOAT", "LONGVARCHAR",
					"SMALLINT", "LONGVARBINARY", "BIGINT" };
			for (int i = 0; i < patterns.length; i++) {
				pos = 0;
				pattern = patterns[i];
				while ((pos = text.indexOf(pattern, pos)) >= 0) {
			...
			while ((pos = text.indexOf("'", pos)) >= 0) {
				...
				editor.getDocument().insertString(pos,
						text.substring(pos, endpos + 1), getStyle("literal"));
			...
			while ((pos = text.indexOf("--", pos)) >= 0) {
				...
				editor.getDocument().insertString(pos,
						text.substring(pos, endpos + 1), getStyle("comment"));
			...
			while ((pos = text.indexOf("/*", pos)) >= 0) {
				...
				editor.getDocument().insertString(pos,
						text.substring(pos, endpos + 2), getStyle("comment"));
